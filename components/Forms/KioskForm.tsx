...
const KioskForm: React.FC = () => {
    ...
    useEffect(() => {
        const fetchForm = async () => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    const data = docSnap.data() as DynamicForm;
                    if (data.isActive) {
    ...
    const handleMultiChoiceChange = (fieldId: string, option: string, isChecked: boolean) => {
        setAnswers(prev => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast("Ocurrió un error al enviar tus respuestas.", "error");
        } finally {
        ...
        return (
            <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center p-6">
                <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 text-center max-w-md w-full">
                    ...
                    <h2 className="text-xl text-gray-900 font-medium mb-2">Aviso</h2>
                    <p className="text-gray-500">{error || "No se pudo cargar el formulario."}</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center p-6">
                <motion.div 
                    ...
                    className="bg-white p-10 rounded-3xl shadow-xl border border-gray-100 text-center max-w-lg w-full"
                >
                    <CheckCircle className="w-24 h-24 text-gold mx-auto mb-6" />
                    <h1 className="text-3xl font-serif text-gray-900 mb-4">¡Gracias!</h1>
                    <p className="text-gray-600 mb-10">Tus respuestas han sido registradas exitosamente.</p>
                    <button 
                        onClick={resetForm}
                        className="px-8 py-4 bg-gold text-black font-bold uppercase tracking-widest rounded-xl hover:bg-gray-100 transition-colors w-full shadow-lg shadow-gold/20"
                    >
                        Siguiente Paciente
                    </button>
                </motion.div>
    ...
    return (
        <div className="min-h-screen bg-[#f9f8f6] text-gray-900 selection:bg-gold/30 relative overflow-hidden">
            {/* Background Accents (matching Products page) */}
            <div className="absolute top-0 left-0 w-96 h-96 bg-gold/10 rounded-full blur-[120px] pointer-events-none"></div>
            <div className="absolute bottom-0 right-0 w-64 h-64 bg-gray-200/50 rounded-full blur-[100px] pointer-events-none"></div>

            {/* Transparent Header */}
            <header className="py-6 px-8 flex justify-center border-b border-gray-200/50 relative z-20">
                <div className="h-8 w-auto text-gold">
                    <svg
                        version="1.2"
            ...
            <main className="max-w-3xl mx-auto px-6 py-12 md:py-20 relative z-10">
                ...
                >
                    <h1 className="text-4xl md:text-5xl font-serif text-black mb-4">{form.title}</h1>
                    {form.description && (
                        <p className="text-gray-500 font-light text-sm md:text-base leading-relaxed max-w-2xl mx-auto">{form.description}</p>
                    )}
                </motion.div>

                <form onSubmit={handleSubmit} className="space-y-10">
                    
                    {/* Identification Section (Optional for tracking) */}
                    <div className="bg-white border border-gray-100 hover:border-gold/50 rounded-xl shadow-lg hover:shadow-xl p-8 transition-all duration-500 space-y-6 group">
                        <h3 className="text-xl text-black font-serif mb-4 border-b border-gray-100 pb-4 group-hover:border-gold/30 transition-colors">Tus Datos (Opcional)</h3>
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-white border border-gray-200 rounded-xl p-4 text-gray-900 focus:border-gold/50 focus:ring-1 focus:ring-gold/20 outline-none transition-all placeholder:text-gray-400"
                    ...
                    {[...form.fields].sort((a, b) => a.order - b.order).map((field) => {
                        console.log('Rendering field:', field.order, field.type, field.label);
                        return (
                        <motion.div 
                            key={field.id}
                            ...
                            className="bg-white border border-gray-100 hover:border-gold/50 rounded-xl shadow-lg hover:shadow-xl p-8 transition-all duration-500 group"
                        >
                            {field.type !== 'image' && (
                                <h3 className="text-xl text-gray-900 font-medium mb-6">
                                    {field.label}
                                    {field.required && <span className="text-gold ml-2">*</span>}
                                </h3>
                            )}
                            ...
                            {field.type === 'textarea' && (
                                <textarea 
                                    ...
                                    value={answers[field.id] || ''}
                                    onChange={(e) => handleFieldChange(field.id, e.target.value)}
                                    className="w-full bg-white border border-gray-200 rounded-xl p-4 text-gray-900 focus:border-gold/50 focus:ring-1 focus:ring-gold/20 outline-none transition-all placeholder:text-gray-400 h-32 resize-none"
                            ...
                            {field.type === 'single_choice' && (
                                <div className="space-y-3">
                                    {field.options?.map((opt, i) => (
                                        <label 
                                            key={i} 
                                            className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${answers[field.id] === opt ? 'bg-gold/10 border-gold text-gold' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300'}`}
                                        >
                                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${answers[field.id] === opt ? 'border-gold' : 'border-gray-300'}`}>
                                                {answers[field.id] === opt && <div className="w-2.5 h-2.5 bg-gold rounded-full"></div>}
                                            </div>
                                            <input 
                                                type="radio"
                                ...
                                <div className="space-y-3">
                                    {field.options?.map((opt, i) => {
                                        ...
                                        return (
                                            <label 
                                                key={i} 
                                                className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${isChecked ? 'bg-gold/10 border-gold text-gold' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300'}`}
                                            >
                                                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${isChecked ? 'border-gold bg-gold' : 'border-gray-300'}`}>
                                                ...
                                                <input 
                                                    ...
                                                    value={opt}
                                                    checked={isChecked}
                                                    onChange={(e) => handleMultiChoiceChange(field.id, opt, e.target.checked)}
                                                    className="hidden"
                                                />
                                                <span className="text-lg">{opt}</span>
                                            </label>
                            ...
                            {field.type === 'image' && field.imageUrl && (
                                <div className="rounded-xl overflow-hidden shadow-xl border border-gray-100">
                                    <img src={field.imageUrl} alt="Apoyo visual" className="w-full h-auto object-cover" />
                                </div>
                            )}
                        </motion.div>
                        );
                    ...
                    <div className="pt-8">
                        <button
                            ...
                            className="w-full relative overflow-hidden bg-gold text-black font-bold uppercase tracking-widest py-5 rounded-2xl transition-all duration-300 hover:shadow-[0_0_40px_rgba(212,175,55,0.4)] disabled:opacity-50 group text-lg flex items-center justify-center gap-2"
                        >
                            {isSubmitting ? (
