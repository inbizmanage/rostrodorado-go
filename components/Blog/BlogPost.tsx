 ...
 * If content already has HTML block elements (<p>, <h2>, etc.), render as-is.
 * If content is plain text with newlines, convert double newlines to paragraphs
 * and single newlines to <br> tags.
 */
const normalizeContent = (raw: string): string => {
    // Check if content already contains HTML block-level elements
    const hasHtmlBlocks = /<(p|h[1-6]|ul|ol|li|div|blockquote|table|section|article)[\.\s>]/i.test(raw);
    if (hasHtmlBlocks) {
        return raw;
    }
...
const BlogPost = () => {
    ...
    useEffect(() => {
        const fetchPost = async () => {
            ...
            try {
                ...
                if (!snapshot.empty) {
                    ...
                    if (metaDesc) metaDesc.setAttribute('content', docData.excerpt);

                    // Increment View Count
                    const docRef = doc(db, 'posts', snapshot.docs[0].id);
                    updateDoc(docRef, { views: increment(1) }).catch(err => console.error("Error updating views", err));
                    // -----------------------------------------------
                }
            } catch (error) {
        ...
        return () => {
            document.title = "Rostro Dorado Clinic | Medicina Estética";
        };
    }, [slug]);
    ...
    if (!post) {
        return (
            <div className="min-h-screen flex flex-col justify-center items-center bg-base">
                <h2 className="text-2xl font-serif mb-4">Artículo no encontrado</h2>
                <Link to="/blog" className="text-gold underline">Volver al Blog</Link>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white min-h-screen pt-28 pb-20">
            {/* Editorial Layout: Title -> Meta -> Image -> Content */}

            <article className="container mx-auto px-4 max-w-5xl">

                {/* Blog Header Branding */}
                <div className="flex flex-col items-center justify-center mb-10 opacity-80 hover:opacity-100 transition-opacity">
                    <svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 825 82" className="h-8 w-auto fill-charcoal" aria-label="Rostro Dorado">
                    ...
                    <span className="font-sans text-[10px] uppercase tracking-[0.4em] mt-3 text-gold font-bold">
                        Blog
                    </span>
                </div>

                {/* Breadcrumb */}
                <div className="flex items-center justify-center gap-2 text-xs md:text-sm text-gray-400 mb-8 uppercase tracking-widest font-sans opacity-60">
                ...
                {/* Editorial Two-Column Header: Image + Title */}
                <motion.div
                ...
                >
                    {/* Square Cover Image */}
                    <div className="aspect-square bg-gray-100 rounded-2xl overflow-hidden shadow-sm order-1">
                        <img
                            src={post.coverImage}
                            alt={post.title}
                    ...
                    <div className="flex flex-col justify-center order-2">
                        <span className="inline-block px-3 py-1 bg-gold/10 text-gold text-xs font-bold tracking-[0.2em] uppercase mb-4 rounded-sm w-fit">
                            Medicina Estética
                        </span>
                        <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl text-charcoal mb-6 leading-tight">
                            {post.title}
                        </h1>

                        <div className="flex items-center gap-4 text-gray-500 font-sans text-sm border-y border-gray-100 py-4 w-fit pr-8">
                            <span className="flex items-center gap-2">
                                <span className="font-bold text-charcoal">{post.author}</span>
                            </span>
                            <span className="w-1 h-1 bg-gray-300 rounded-full"></span>
                            <span>{new Date(post.createdAt?.seconds * 1000).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                        </div>
                    </div>
                </motion.div>

                {/* Content Container - Two Column on Large Screens or Centered Single */}
                <div className="max-w-3xl mx-auto">
                    <div
                        className="prose prose-lg prose-headings:font-serif prose-headings:text-charcoal prose-p:text-gray-900 prose-p:leading-8 prose-a:text-gold prose-blockquote:border-l-gold prose-blockquote:bg-gold/5 prose-blockquote:py-2 prose-blockquote:px-6 prose-blockquote:not-italic prose-blockquote:font-serif prose-img:rounded-lg"
                    ...
                    <div className="mt-20 pt-10 border-t border-gray-100 flex justify-between items-center">
                        <Link to="/blog" className="group flex items-center gap-3 text-charcoal hover:text-gold transition-colors font-serif text-lg">
                            <span className="p-2 border border-gray-200 rounded-full group-hover:border-gold transition-colors">
                                <ArrowLeft size={16} />
                            </span>
                            Leer más artículos
                        </Link>
